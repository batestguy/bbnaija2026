...
lists for all 11 interpreters) and `raw\r_packages.csv` (all 1,027 R packages).
Scanned 2026-08-05; re-verified 2026-09-07 (new `drlca-rag` env, sizes, key versions).
Only packages that *discriminate* between envs are listed below —
...
|---|---|---|---|
| `ds-general` | `conda activate C:\Users\TOSHIBA\ds-general` | 3.12 | 3.4 GB |
| `causality-handbook` | `conda activate causality-handbook` | 3.11 | 4.4 GB |
...
| `drlca-rag` | `conda activate C:\conda-envs\drlca-rag` | 3.11 | 1.1 GB |
...
| RAG / PDF ingest + vector retrieval + Gemini + demo | `drlca-rag` |
...
and `torch 2.10` (**CPU-only** `cpu_mkl` build — verified, no CUDA). Still the only
env with PyTorch (`drlca-rag` has none).
...
**`drlca-rag`** — lean RAG starter (113 packages, pip-installed on conda Python 3.11),
not a full RAG stack. PDF ingest: `pypdf 6.17` `pymupdf 1.28`
`rapidocr-onnxruntime 1.4` (+ `onnxruntime`, `opencv`, `shapely` — scanned-PDF/OCR
pipeline). Retrieval: `faiss-cpu 1.15` only. LLM/embeddings: `google-genai`
(Gemini API — needs a key at runtime, nothing local). Demo: `streamlit 1.63`.
*Missing vs a full stack:* no `langchain`/`llama-index`, no `sentence-transformers`,
no `torch`/`transformers`, no chroma/qdrant. *Avoid:* CPU-only faiss; anything
needing local embeddings or an orchestration framework must be installed first.
Not used by the BBNaija project (blogs + PyMC need no retrieval).
...
- **Two envs are prefix envs outside `miniconda3\envs\`:** `ds-general` at
  `C:\Users\TOSHIBA\ds-general` and `drlca-rag` at `C:\conda-envs\drlca-rag`.
  `conda list -n <name>` **fails** for both; use `-p <full-prefix>`.
  Safest habit: use `-p <prefix>` everywhere. (Same rule covers
  `conda run` and `conda activate`.)
...
- **numpy is split across a major version.** 1.x: `bap3` (1.24), `ds-general`,
  ...
  `base`, system (2.4–2.5), `drlca-rag` (2.4). Code written for one side can break
  on the other.
- **pandas 3.x is not everywhere it was assumed.** Re-verified 2026-09-07:
  `causality-handbook` measures **2.3.3** (not 3.x); `bap3` is on 2.1.2. Treat any
  3.x claim for an env as unverified until `conda list` says so — expect API
  differences, especially around copy-on-write and string dtype.
