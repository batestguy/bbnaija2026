...
## Current status — P0–P8 COMPLETE; season ops mode (as of 2026-09-17)
...
- **Fan-poll tracking: TRACK-ONLY (owner decision 2026-09-18):** unofficial polls (bbnaijadaily.com widget — up to 100 repeat votes/person; ngnews247 YouTube poll videos; FB voting groups) are logged by hand in `docs/polls.json` next to predictions. **Never scraped into the pipeline, never a CPI/model input.** Week-8 scored: hazard concordance 38/55 pairs (0.691 vs 0.5 coin-flip); poll top-3 [Keivo, Temi Nkem, Ricky] vs model P(#1) top-3 [Temi Nkem, Chimsom Chuka, Flora]. Agreements: poll #1 (Keivo) == model's safest nominated (hazard 0.69); poll #11 (Chimsom Chuka) == model's most at-risk (1.82). The winner-model disagreement (quiet fanbases like Keivo/Ricky poll high, high-coverage housemates poll low) is the documented coverage-vs-vote-intensity limitation of CPI — known, stated, not patched.
- **Remaining:** Saturday ritual (run → review → push) on Sep 19/26 + Oct 3 — first post-fix run (Sep 19) must verify Yusuf's win prob is nonzero and spot-check medians + BMA weights; tracker auto-scoring; optional P9 rehearsal.
