...
## Current status — P7 dashboard built on placeholder data; P5 validation open (as of 2026-09-13)
- **GitHub repo: https://github.com/batestguy/bbnaija2026** (public, `main` tracked). Season 11 is LIVE (premiered 26 Jul 2026; finale ≈ 4 Oct). See `docs/season-recon.md` + `docs/p5-handoff.md` for the full P5 state.
- **Committed:** P0–P4 (fold-in, recon — Gambit Flora+Aikou wks 1–5, released wk 6, exit ledger reconciled → 16 active; scaffold; scrapers; preprocessing). **P5 WIP committed** (`b8295f9`): model + 38 tests green, but the 4×2000 synthetic validation **fails at the R-hat gate (NaN) with 28/232/36 divergences** — sum-to-zero offsets already cut them ~2×; failure log + ordered recovery plan in `docs/p5-handoff.md`.
- **P7 done (placeholder):** `docs/index.html` + `docs/assets/script.js` render the full weekly-standings spec (trajectory + 89% CrI bands, podium strip, rank-probability chips, Gambit banner, trend projection, at-risk hazards, head-to-head from the new `pairwise` field, roster, DEMO/precision/staleness badges, methodology footer) — QA'd in Chrome. Data via `run_prior_placeholder()` (prior predictive → same assembly path, ~18 s, labelled `precision: "prior-predictive"` + `placeholder: true`; **never publish as predictions**).
- **Next:** (1) NaN-R-hat diagnosis + divergence fix, (2) right-size the e2e pytest, (3) P8 deploy workflow (Pages + HF Space), (4) housemate photos, (5) P6 `run_weekly.py`.
